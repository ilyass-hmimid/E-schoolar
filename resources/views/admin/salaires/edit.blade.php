...
<div class="container mx-auto px-4 py-6">
    <div class="mb-6">
        ...
        <p class="text-gray-600">Mettez à jour les informations du salaire</p>
    </div>
        ...
        <form action="{{ route('admin.salaires.update', $salaire) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <select name="professeur_id" id="professeur_id" required
                        ...
                        @foreach($professeurs as $professeur)
                            <option value="{{ $professeur->id }}" 
                                {{ old('professeur_id', $salaire->professeur_id) == $professeur->id ? 'selected' : '' }}>
                                {{ $professeur->name }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <select name="matiere_id" id="matiere_id"
                        ...
                        @foreach($matieres as $matiere)
                            <option value="{{ $matiere->id }}" 
                                {{ old('matiere_id', $salaire->matiere_id) == $matiere->id ? 'selected' : '' }}>
                                {{ $matiere->nom }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="month" name="mois_periode" id="mois_periode" required
                        value="{{ old('mois_periode', $salaire->mois_periode->format('Y-m')) }}"
                        class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                    @error('mois_periode')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="number" name="heures_travaillees" id="heures_travaillees" required
                        value="{{ old('heures_travaillees', $salaire->heures_travaillees) }}" step="0.5" min="0"
                        class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                ...
                <div>
                    ...
                    <input type="number" name="taux_horaire" id="taux_horaire" required
                        value="{{ old('taux_horaire', $salaire->taux_horaire) }}" step="0.01" min="0"
                        class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                    @error('taux_horaire')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="number" name="montant_total" id="montant_total" required
                        value="{{ old('montant_total', $salaire->montant_total) }}" step="0.01" min="0"
                        class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                    @error('montant_total')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select name="statut" id="statut" required
                        ...
                        <option value="annule" {{ old('statut', $salaire->statut) == 'annule' ? 'selected' : '' }}>Annulé</option>
                    </select>
                    @error('statut')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
            ...
            <div class="mt-6">
                ...
                <textarea name="notes" id="notes" rows="3"
                    class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ old('notes', $salaire->notes) }}</textarea>
                @error('notes')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mt-8 flex justify-between items-center">
                <div>
                    <a href="{{ route('admin.salaires.show', $salaire) }}" class="text-blue-600 hover:text-blue-800 mr-4">
