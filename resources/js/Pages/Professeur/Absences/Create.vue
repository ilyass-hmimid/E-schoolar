    ...
    <div class="bg-white shadow">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div class="flex items-center justify-between">
          <div>
            <h1 class="text-3xl font-bold text-gray-900">Enregistrer une absence</h1>
            <p class="mt-2 text-gray-600">Ajouter une nouvelle absence pour un élève</p>
          </div>
        </div>
      </div>
    </div>
      ...
      <form @submit.prevent="submit">
        <div class="bg-white shadow overflow-hidden sm:rounded-lg">
          <div class="px-4 py-5 sm:p-6 space-y-6">
              ...
              <div class="flex">
                ...
                <div class="ml-3">
                  <p class="text-sm text-red-700">
                    Impossible d'enregistrer l'absence. Veuillez corriger les erreurs ci-dessous.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div>
              <label for="etudiant_id" class="block text-sm font-medium text-gray-700">Étudiant <span class="text-red-500">*</span></label>
              <select
                id="etudiant_id"
                v-model="form.etudiant_id"
                class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md"
                :class="{ 'border-red-300 text-red-900': form.errors.etudiant_id }"
              >
...
const form = useForm({
  etudiant_id: '',
  matiere_id: '',
  date_absence: new Date().toISOString().split('T')[0], // Date du jour par défaut
  heure_debut: '08:00',
  heure_fin: '10:00',
  type: 'absence',
  duree_retard: 15,
...
const submit = () => {
  form.post(route('professeur.absences.store'), {
    onSuccess: () => {
      form.reset();
    },
    preserveScroll: true,
