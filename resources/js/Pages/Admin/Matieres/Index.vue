        ...
        <div class="p-6 overflow-hidden bg-white shadow-sm sm:rounded-lg">
          ...
          <div class="grid grid-cols-1 gap-4 mb-6 sm:grid-cols-2 lg:grid-cols-5">
            ...
            <div>
              ...
              >
                ...
                <option v-for="(label, key) in matiereTypes" :key="key" :value="key">
                  {{ label }}
                </option>
              </select>
            </div>
          ...
          <div class="grid grid-cols-1 gap-5 mb-6 sm:grid-cols-2 lg:grid-cols-4">
            <div class="px-4 py-5 overflow-hidden bg-white rounded-lg shadow sm:p-6">
              <dt class="text-sm font-medium text-gray-500 truncate">Total des matières</dt>
            ...
            <div class="px-4 py-5 overflow-hidden bg-white rounded-lg shadow sm:p-6">
              ...
              <dd class="mt-1 text-3xl font-semibold text-green-600">{{ stats.actives }}</dd>
            </div>
            
            <div class="px-4 py-5 overflow-hidden bg-white rounded-lg shadow sm:p-6">
              <dt class="text-sm font-medium text-gray-500 truncate">Prix moyen (élève)</dt>
              <dd class="mt-1 text-3xl font-semibold text-blue-600">{{ stats.prix_moyen.eleve.toFixed(2) }} DH</dd>
            </div>
            
            <div class="px-4 py-5 overflow-hidden bg-white rounded-lg shadow sm:p-6">
              <dt class="text-sm font-medium text-gray-500 truncate">Rémunération moyenne (prof)</dt>
              <dd class="mt-1 text-3xl font-semibold text-purple-600">{{ stats.prix_moyen.professeur.toFixed(2) }} DH</dd>
            </div>
          </div>
...
<th
                  ...
                  <th
                    scope="col"
                    class="px-6 py-3 text-xs font-medium tracking-wider text-left text-gray-500 uppercase cursor-pointer hover:bg-gray-50"
                    @click="sortBy('est_actif')"
                  >
                    <div class="flex items-center">
                      <span>Statut</span>
                      <span v-if="filters.sort_field === 'est_actif'" class="ml-1">
                        <svg v-if="filters.sort_direction === 'asc'" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 15l7-7 7 7" />
                        </svg>
                        <svg v-else class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <tbody class="bg-white divide-y divide-gray-200">
                <tr v-for="matiere in matieres.data" :key="matiere.id">
                  ...
                  <td class="px-6 py-4 whitespace-nowrap">
                    <span class="inline-flex px-2 text-xs font-semibold leading-5 text-blue-800 bg-blue-100 rounded-full">
                      {{ formatMatiereType(matiere.type) }}
                    </span>
                  </td>
                  <td class="px-6 py-4">
                    <div class="flex flex-wrap gap-1">
                      <span
                        v-for="niveau in matiere.niveaux"
                        :key="niveau.id"
                  ...
                  <td class="px-6 py-4 whitespace-nowrap">
                    ...
                    <div class="text-sm text-gray-500">
                      {{ matiere.prix_prof }} DH (prof)
                    </div>
                  </td>
                  <td class="px-6 py-4 whitespace-nowrap">
                    <span
                      :class="{
                        'bg-green-100 text-green-800': matiere.est_actif,
                        'bg-red-100 text-red-800': !matiere.est_actif
                      }"
                      class="inline-flex px-2 text-xs font-semibold leading-5 rounded-full"
                    >
                      {{ matiere.est_actif ? 'Active' : 'Inactive' }}
                    </span>
                  </td>
                ...
                <tr v-if="matieres.data.length === 0">
          ...
          <!-- Pagination -->
          <Pagination :links="matieres.links" class="mt-4" />
        </div>
      </div>
    </div>
...
const filters = ref({
  niveau_id: props.filters.niveau_id || null,
...
// Types de matières depuis le modèle PHP
const matiereTypes = {
...
// Formater le type de matière pour l'affichage
const formatMatiereType = (type) => {
  return matiereTypes[type] || type;
};

const confirmDelete = (matiere) => {
  selectedMatiere.value = matiere;
  showDeleteModal.value = true;
};

const deleteMatiere = () => {
  deleteForm.delete(route('admin.matieres.destroy', selectedMatiere.value.id), {
    preserveScroll: true,
    onSuccess: () => {
...
const sortBy = (field) => {
  ...
  router.get(route('admin.matieres.index'), 
    { 
      ...filters.value,
      sort_field: field,
...
// Filtrer les matières avec debounce
const filterMatieres = debounce(() => {
  router.get(route('admin.matieres.index'), 
    { 
      ...filters.value,
      search: search.value,
